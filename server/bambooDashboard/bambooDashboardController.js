...
let riglets = require('../rigletSchema.js');
...
let getTools = (username, rigletname, successCB, errorCB) => {
       riglets.findOne({userName:username,rigletName:rigletname},{rigTools:1},function(err,riglet) {
          if(err) {
            console.log("MONGOOOOAOOSDAOSDOOOOOO",err)
            errorCB(err);
          } else {
            let toolsArray = [];
            riglet.rigTools.map((server, rigindex) => {
                server.tools.map((tool, toolindex) => {
                  toolsArray.push(tool);
                });
            });
            successCB(toolsArray);
          }
        });
}
